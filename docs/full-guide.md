...
> - 日股/韩股/台股：当前仅走 Yfinance 基础路径获取日线与实时行情；`institution`、`capital_flow`、`dragon_tiger`、`boards` 等依赖 A 股专属源/离岸完整版的能力会降级为 `not_supported`（详见 [市场支持与边界](market-support.md)）；
...
| 台股 | Yahoo 后缀 `.TW`（上市）/ `.TWO`（上柜） | `2330.TW`、`6488.TWO` |
